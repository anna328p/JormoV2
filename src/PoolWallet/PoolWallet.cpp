...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 10 shells = 0.1 TRTL */
  if (cmd.fee == 10) {
    feeString = "0.1 JOR (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " JOR";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " JOR, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " JOR" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string jor = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << jor << " JOR belongs to you!";
    }
  }
  ...
  {
    std::string jor = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << jor << " JOR were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "JormoCoin v" << PROJECT_VERSION << " Poolwallet" << std::endl;

  std::cout << "Please note that usage of simplewallet/poolwallet has been "
            << "deprecated for pool usage." << std::endl
            << "If you are using jormo-pool, you can trivially transfer to "
            << "walletd by following these instructions:" << std::endl
            << "https://github.com/JormoCoin/JormoV2" << std::endl;
