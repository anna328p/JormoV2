            ...
            switch (e.code().value())
            {
                ...
                case NodeErrors::CryptoNote::error::CONNECT_ERROR:
                {
                    std::cout << WarningMsg("Couldn't connect to the network "
                              ...
                              << std::endl
                              << "Ensure jormocoind or the remote node you "
...
Maybe<uint64_t> getTransferAmount()
{
    while (true)
    {
        ...
        std::cout << std::endl
                  << InformationMsg("How much JOR do you want to send?: ");
...
bool parseFee(std::string feeString)
{
    ...
    {
        std::cout << WarningMsg("Fee must be at least 0.1 JOR!") << std::endl;
        return false;
    }
...
bool parseAddress(std::string address)
{
    ...
    else if (expectedPrefix == 3914525 && address.substr(0, 4) != "JOR")
    {
        std::cout << WarningMsg("Invalid address! It should start with JOR!")
                  << std::endl << std::endl;
        return false;
    }
...
bool parseAmount(std::string amountString)
{
    ...
    if (!parseAmount(amountString, amount))
    {
        std::cout << WarningMsg("Failed to parse amount! Ensure you entered "
                                "the value correctly.")
                  << std::endl
                  << "Please note, the minimum you can send is 0.01 JOR,"
                  << std::endl
